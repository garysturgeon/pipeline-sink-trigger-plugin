...
A custom Jenkins plugin primarily used for periodically scheduling a build of a configured sink job if and only if the corresponding build pipeline graph is 
...
Using the above example, we see that **Component 1** (which is built by Jenkins - represented by the **Job 1** node) has a dependency on **Component Root**
(which is built by Jenkins - represented by the **Job (Root)** node). Furthermore, we see that **Component 2**, **Component 3**, and **Component 4** have
...
of Jenkins's built in support for parallelized builds. The branching can extend as deep as necessary.

Another observation that cannot escape our attention is how all component build jobs converge on **Job (Sink)**. This **Job (Sink)** represents the Jenkins
...
Out of the box, Jenkins supports triggering builds of the other projects (once a build is successfully completed) thereby constructing, and managing the
...
1.  Jenkins will trigger/schedule a build of **Component 1** using the **Job 1** configuration (note: here we are assuming our jobs are configured to poll 
changes in SCM).
2.  **Component 1** gets built (i.e. compile/test/etc.).
3.   Jenkins triggers the downstream builds of **Component 2**, **Component 3**,  **Component 4**, and **Job (Sink)** (remember that **Job (Sink)** represents 
the Jenkins build job for our software product that we intend to deliver).
4.  **Component 2** gets built (for the purposes of illustration ignore build order queue).
5.  Jenkins triggers the downstream builds for **Component 2**: **Component 2.1**, **Component 2.2**,  and **Job (Sink)** (again!!).
...
has been detected, yet try to avoid unnecessary repetive builds (Jenkins has no support for this - this plugin aims at providing this mechanism).

## Configuration

Create a new Jenkins job configuration (for example MyProduct-Trigger-Observer), and configure the following _Build Trigger_:
...
Jenkins server using this file.
    
To run locally:

    mvn hpi:run

Then you can access the embedded Jenkins instance at [http://localhost:8080/](http://localhost:8080/).
